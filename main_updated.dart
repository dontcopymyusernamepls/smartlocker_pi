...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<Map<String, dynamic>?> fetchLockerStats() async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        itemCount: parcels.length, // changed from parcels.length + 1
              ...
              child: Column(
                ...
                children: [
                  ...
                  FutureBuilder<Map<String, dynamic>?>(
                    future: fetchLockerStats(),
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return const LinearProgressIndicator();
                      } else if (snapshot.hasError || !snapshot.hasData) {
                        return const Text(
                          'Failed to load locker stats.',
                      ...
                      return Column(
                        ...
                        children: [
                          ...
                          Text(
                            '🔻 Temperature: ${stats['temperature']?.toStringAsFixed(1) ?? '-'} °C',
                            style: const TextStyle(fontSize: 14),
                          ),
                          Text(
                            '💧 Humidity: ${stats['humidity']?.toStringAsFixed(1) ?? '-'} %',
                            style: const TextStyle(fontSize: 14),
                          ),
                          Text(
                            '📦 Locker Empty: ${stats['locker_empty'] ?? '-'}',
                            style: const TextStyle(fontSize: 14),
                          ),
                          if (stats['message'] != null &&
                              stats['message'].toString().isNotEmpty)
                            Padding(
