...
def on_mqtt_message(client, userdata, msg):
    ...
    try:
        data = json.loads(payload)  # Parse MQTT payload as JSON
        
        if msg.topic == MQTT_TOPIC_SENSORS:
            print(f"Forwarding sensor data to Firebase/WS: {payload}")
            
            # Update Firebase Realtime DB under "Sensors_data"
            sensor_ref.update(data)  # Updates fields without overwriting
            ...
            print(f"Updating door status in Firebase: {payload}")
            
            # Update Firebase under "Locker_status/door"
            lockerStatus_ref.child("door").update(data)  # Example: {"locked": false}
            
    except json.JSONDecodeError:
        print(f"Invalid JSON in MQTT payload: {payload}")
    except Exception as e:
