...
buzzer = 17
Relay = 27
parcel_sensor = 26  # IR Sensor GPIO

relayState = True
parcel_present = False  # Track previous parcel state
...
GPIO.setup(parcel_sensor, GPIO.IN)  # IR sensor as input
...
def check_parcel_presence():
    global parcel_present
    current_state = GPIO.input(parcel_sensor) == GPIO.LOW

    if current_state and not parcel_present:
        lcd.lcd_clear()
        lcd.lcd_display_string("Parcel detected", 1, 0)
        print("[IR SENSOR] Parcel present.")
        sleep(2)
        lcd.lcd_clear()
        parcel_present = True
    elif not current_state and parcel_present:
