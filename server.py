...
def locker_statistics():
    try:
        ...
        if os.path.exists(ir_file_path):
            ...
            placed_at = ir_data.get("placed_at")

            # Check if parcel is overdue (in locker > 3 days)
            if locker_empty == "No" and placed_at:
                try:
                    placed_time = datetime.datetime.fromisoformat(placed_at)
                    now = datetime.datetime.now()
                    if (now - placed_time) > datetime.timedelta(seconds=30):  # use seconds=30 for testing
                        overdue_message = "Parcel has been in locker for more than 3 days."
                except Exception as e:
                    print(f"[ERROR] Failed to parse placed_at: {e}")
        ...
        if overdue_message:
            response["message"] = overdue_message
